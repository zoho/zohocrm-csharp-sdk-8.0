                            ...
                            if (value is IList)
                            {
                                ...
                                if (requestObjects.Count > 0)
                                {
                                    if (memberDetail == null || (memberDetail != null && !memberDetail.ContainsKey(Constants.STRUCTURE_NAME)))
                                    {
                                        ...
                                        {
                                            jsonArray.Add(JsonConvert.SerializeObject(converter.FormRequest(requestObject, (string)memberDetail.GetValue(Constants.STRUCTURE_NAME), null, null)));
                                        }
                                    }
                                }
                                return jsonArray.ToString();
                            }
                            return JsonConvert.SerializeObject(converter.FormRequest(value, (string)memberDetail.GetValue(Constants.STRUCTURE_NAME), null, null)).ToString();
                        }
                        else if(memberDetail.ContainsKey(Constants.TYPE) && ((string)memberDetail.GetValue(Constants.TYPE)).Equals(Constants.CHOICE_NAMESPACE))
					    {
