    ...
    public class DBStore : ITokenStore
    {
        ...
        public IToken FindToken(IToken token)
        {
            try
            {
                ...
                if (oauthToken.UserSignature != null)
                {
                    ...
                    {
                        query.Append(" where user_name = @user_name");
                        param = name;
                        key = "@user_name";
                    }
                }
                ...
                {
                    query.Append(" where access_token = @access_token");
                ...
                else if ((oauthToken.RefreshToken != null || oauthToken.GrantToken != null) && oauthToken.ClientId != null && oauthToken.ClientSecret != null)
                {
                    ...
                    {
                        query.Append(" where grant_token = @grant_token");
                        param = oauthToken.GrantToken;
                        key = "@grant_token";
                    }
                    else if (oauthToken.RefreshToken != null && oauthToken.RefreshToken.Length > 0)
                    {
                        query.Append(" where refresh_token = @refresh_token");
                        param = oauthToken.RefreshToken;
                        key = "@refresh_token";
                    }
                }
                ...
                using (MySqlConnection connection = new MySqlConnection(this.connectionString))
                {
                    ...
                    {
                        statement.Parameters.AddWithValue(key, param);
        ...
        public void SaveToken(IToken token)
        {
            ...
            try
            {
                using (MySqlConnection connection = new MySqlConnection(this.connectionString))
                {
                    ...
                    {
                        query.Append(SetToken(oauthToken)).Append(" where access_token = @access_token");
                        param = oauthToken.AccessToken;
                        key = "@access_token";
                    }
                    else if (((oauthToken.RefreshToken != null && oauthToken.RefreshToken.Length > 0) || (oauthToken.GrantToken != null && oauthToken.GrantToken.Length > 0)) && oauthToken.ClientId != null && oauthToken.ClientSecret != null)
                    {
                        if (oauthToken.GrantToken != null && oauthToken.GrantToken.Length > 0)
                        {
                            query.Append(SetToken(oauthToken)).Append(" where grant_token = @grant_token");
                            param = oauthToken.GrantToken;
                            key = "@grant_token";
                        }
                        else if (oauthToken.RefreshToken != null && oauthToken.RefreshToken.Length > 0)
                        {
                            query.Append(SetToken(oauthToken)).Append(" where refresh_token = @refresh_token");
                            param = oauthToken.RefreshToken;
                            key = "@refresh_token";
                        }
                    }
        ...
        public void DeleteToken(string id)
        {
            try
            {
                ...
                {
                    connection.Open();
                    string query = new StringBuilder().Append("delete from ").Append(this.tableName).Append(" where id = @id").ToString();
