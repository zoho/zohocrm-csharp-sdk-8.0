...
namespace Com.Zoho.Crm.API.Util
{
    ...
    public abstract class Converter
    {
        ...
        /// <param name="pack">A string containing the expected method return type.</param>
        /// <returns>A list of object representing the POJO class instance.</returns>
        public abstract List<Object> GetWrappedResponse(object response, string pack);
        ...
        public bool ValueChecker(string className, string memberName, JObject keyDetails, object value, Dictionary<string, List<object>> uniqueValuesMap, int? instanceNumber)
        {
            ...
            if (keyDetails.ContainsKey(Constants.VALUES) && (!keyDetails.ContainsKey(Constants.PICKLIST) || ((bool)keyDetails[Constants.PICKLIST] && Initializer.GetInitializer().SDKConfig.PickListValidation)))
            {
                ...
                CheckChoiceValue(memberName, className, instanceNumber, valuesJArray, value, detailsJO, pickListValue);
            }
        ...
        public void CheckChoiceValue(String className, JObject keyDetails, Object value)
        {
            String name = Convert.ToString(keyDetails.GetValue(Constants.NAME));
            if (keyDetails.ContainsKey(Constants.VALUES) && name != null && Initializer.GetInitializer().SDKConfig.PickListValidation)
            {
                JArray valuesJArray = (JArray)keyDetails.GetValue(Constants.VALUES);
                string[] pickListValue = valuesJArray.ToObject<string[]>();
                CheckChoiceValue(name, className, 0, valuesJArray, value, new JObject(), pickListValue);
            }
        }

        public void CheckChoiceValue(String memberName, String className, int? instanceNumber, JArray valuesJArray, Object value, JObject detailsJO, string[] pickListValue)
        {
            if (value is IList)
            {
