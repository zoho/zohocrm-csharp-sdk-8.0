    ...
    public class GetBulkWriteJobDetails
    {
        public static void GetBulkWriteJobDetails_1(string jobId)
        {
            ...
            if (response != null)
            {
                ...
                {
                    BulkWriteResponse bulkWriteResponse = (BulkWriteResponse)response.Object;
                    ...
                    if (resources != null)
                    {
                        foreach (Resource resource in resources)
                        {
                            ...
                            Console.WriteLine("Bulk write Resource Find By: " + resource.FindBy);
                    ...
                    if (callBack != null)
                    {
                        ...
                        Console.WriteLine("Bulk write Callback Method: " + callBack.Method);
                    }
                    Console.WriteLine("Bulk write Job ID: " + bulkWriteResponse.Id);

                    Result result = bulkWriteResponse.Result;
                    if (result != null)
                    {
                        Console.WriteLine("Bulk write Job Result Download URL: " + result.DownloadUrl);
                    }
                    Com.Zoho.Crm.API.Users.MinifiedUser createdBy = bulkWriteResponse.CreatedBy;
                    if (createdBy != null)
                    {
                        Console.WriteLine("Bulk write Created By User-Name: " + createdBy.Name);
                        Console.WriteLine("Bulk write Created By User-ID: " + createdBy.Id);
                    }
                    Console.WriteLine("Bulk write Job Operation: " + bulkWriteResponse.Operation);
                    Console.WriteLine("Bulk write Job Created Time: " + bulkWriteResponse.CreatedTime);
                }
                else if (response.Object is APIException)
                {
                    APIException exception = (APIException)response.Object;
