        ...
        public APIResponse<T> APICall<T>(Type className, string encodeType)
        {
            ...
            try
            {
                connector.AddHeader(Constants.ZOHO_SDK,  Environment.OSVersion.Platform.ToString() + "/" +
                                                    Environment.OSVersion.Version.ToString() + "/csharp-8.0/" +
                ...
                if (!string.IsNullOrEmpty(mimeType) && !string.IsNullOrWhiteSpace(mimeType))
                {
                    ...
                    List<Object> responseObject = (List<Object>)convertInstance.GetWrappedResponse(response, pack);
                    if (responseObject != null)
                    {
                        if (responseObject[0] != null)
                        {
                            returnObject = (Model)responseObject[0];
                            if (returnObject != null && (pack.Equals(returnObject.GetType().FullName) || IsExpectedType(returnObject, pack)))
                            {
                                isModel = true;
                            }
                        }
                        if (responseObject.Count == 2 && responseObject[1] != null)
                        {
                ...
                return new APIResponse<T>(headerMap, statusCode, returnObject, isModel, statusDescription, responseJSON);
            }
            catch (SDKException e)
            {
